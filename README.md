...
### Налаштування сервера (обов'язково)

Перед першим деплоєм потрібно налаштувати sudo без пароля для SSH користувача:

1. Підключіться до вашого VPS через SSH
2. Відредагуйте sudoers файл:
   ```bash
   sudo visudo
   ```
3. Додайте рядок (замініть `your_user` на вашого SSH користувача):
   ```
   your_user ALL=(ALL) NOPASSWD: ALL
   ```
4. Збережіть та вийдіть

**Альтернатива:** Якщо не хочете давати повні права sudo, можна обмежити тільки для Docker:
   ```
   your_user ALL=(ALL) NOPASSWD: /usr/bin/docker, /usr/local/bin/docker, /usr/bin/docker-compose, /usr/local/bin/docker-compose
   ```
...
**Примітка:** 
- SSH ключ та .env файл зберігаються в GitHub Secrets, тому їх не потрібно вводити щоразу
- n8n буде встановлено в `~/n8n` (домашня директорія користувача)
